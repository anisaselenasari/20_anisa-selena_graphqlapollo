...
import { gql, useQuery, useLazyQuery} from '@apollo/client'   ///usequery adl hooks yg punya apollo , use query buat returnnya itu objek



// baris ke 8 itu berfungsi untuk copy query yg ada di hasura
...
function TodoList() {
  const {data, loading, error} = useQuery(GetTodolist);    //kita distructring disini
  ...
  // const onclick item itu berfungsi sbg, ketika user mencet todo, dia akan kek nunjukin bisa yg awalnya true dan bisa false
  ...
  ////// ngirim id, abis itu di filter untuk semua element yang id nya tidak sama degan yang kita kirim//////////////////////////////
  ...
  return (
    <>
      <div className='container'>
        ...
        <ul className='todo-list js-todo-list'>
          {data?.todolist.map((v, i) => (
            <Todo
              key={v.id}
              id={v.id}
              onClickItem={() => onClickItem(i)}       //dikirm sebagai props
              onDeleteItem={() => onDeleteItem(i)}     // dikirim sebagai props //variabel nya iyu (i), index nya dimulai dari 0
              title={v.title}
              checked={v.is_done}
            />
          ))}
        ...
        {/* kita buat form class input,
        baris ke 61 hubungan nya sama baris ke 7.8.9 ... ketika user mencet enter baru menjalankan on submit 
        jadi baris ke 60 hubungannya sama baris 15  , agar dapat menambahkan objek baru di list*/}
